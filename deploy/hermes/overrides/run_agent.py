...
class AIAgent:
    ...
    def _compute_non_stream_stale_timeout(self, api_payload: Any) -> float:
        ...
        if (
            ...
            and base_url
            and not getattr(self, "_xiaochuang_model_gateway", False)
    ...
    def _create_request_openai_client(self, *, reason: str, api_kwargs: Optional[dict] = None) -> Any:
        ...
        xiaochuang_context_tokens = []
        try:
            from gateway.xiaochuang_runtime_context import (
                bind_xiaochuang_runtime_context_reference,
            ...
            model_base_url = request_kwargs.get("base_url", "")
            if is_xiaochuang_model_gateway_base_url(model_base_url):
                active_context = get_xiaochuang_runtime_context()
                if not active_context.get("capability_token"):
                    runtime_context_reference = getattr(
                        self,
                        "xiaochuang_runtime_context_reference",
                        None,
                    )
                    if isinstance(runtime_context_reference, dict):
                        xiaochuang_context_tokens = (
                            bind_xiaochuang_runtime_context_reference(
                                runtime_context_reference
                            )
                        )
                    elif getattr(self, "xiaochuang_capability_token", ""):
                        # Compatibility for a managed Agent created before
                        # the private reference was introduced.
                        xiaochuang_context_tokens = (
                            set_xiaochuang_runtime_context(
                ...
                xiaochuang_headers = get_xiaochuang_model_gateway_headers(
                    model_base_url
                )
                refreshed_context = get_xiaochuang_runtime_context()
                self.xiaochuang_capability_token = refreshed_context[
                    "capability_token"
                ]
            else:
                xiaochuang_headers = {}
            ...
            if xiaochuang_context_tokens:
                clear_xiaochuang_runtime_context(xiaochuang_context_tokens)
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        try:
            ...
            xiaochuang_headers = get_xiaochuang_model_gateway_headers(base_url)
        except ImportError:
            xiaochuang_headers = {}
        if xiaochuang_headers:
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict, model: "str | None" = None) -> dict:
        ...
        model_consumes_thought_signature = (
            "gemini" in normalized_model
        )
        if not model_consumes_thought_signature:
