...
XIAOCHUANG_DRAMA_TOOLSET = "xiaochuang-drama"
XIAOCHUANG_SKILL_MANIFEST_HEADER = "X-Xiaochuang-Skill-Manifest"
XIAOCHUANG_TOOL_PROFILE_ALLOWED_TOOLS = {
    "xiaochuang-drama-source": frozenset({
        "get_task_context",
        "list_source_chunks",
        "get_source_chunk",
    ...
    "xiaochuang-drama-plan": frozenset({
        "get_task_context",
        "list_source_chunks",
        "get_source_chunk",
    ...
    "xiaochuang-drama-script": frozenset({
        "get_task_context",
        "list_source_chunks",
        "get_source_chunk",
    ...
    "xiaochuang-drama-graph": frozenset({
        "get_task_context",
    ...
    "xiaochuang-drama-storyboard": frozenset({
...
XIAOCHUANG_RUN_HEADER_NAMES = (
    ...
    XIAOCHUANG_SKILL_MANIFEST_HEADER,
)


def _xiaochuang_api_server_toolsets() -> set[str]:
    """Return the exact deployment-declared toolsets for managed runs.

    Managed Xiaochuang containers intentionally do not use Hermes's general
    platform-toolset resolver. That resolver is designed for interactive
    installs and can auto-enable convenience toolsets based on local
    credentials (for example ``x_search`` when ``XAI_API_KEY`` exists).
    A managed container instead treats the explicit ``api_server`` list as an
    allowlist and lets the caller reject anything other than the business MCP
    bridge.
    """
...
def _validate_xiaochuang_drama_toolsets(enabled_toolsets: set[str]) -> None:
    """Fail closed unless a Xiaochuang run can only see its business toolset."""
    if enabled_toolsets != {XIAOCHUANG_DRAMA_TOOLSET}:
        enabled = ", ".join(sorted(enabled_toolsets)) or "(none)"
        raise ValueError(
            "Xiaochuang managed runs require api_server toolsets to be exactly "
            f"{XIAOCHUANG_DRAMA_TOOLSET!r}; got {enabled}"
        )


def _xiaochuang_allowed_tool_names(tool_profile: str) -> frozenset[str]:
    """Return the exact model-visible tool surface for a managed run."""
    return XIAOCHUANG_TOOL_PROFILE_ALLOWED_TOOLS.get(
        str(tool_profile or "").strip(),
        frozenset(),
    )


def _validate_xiaochuang_deployment_tool_profile(tool_profile: str) -> None:
    """Keep a dedicated Hermes container pinned to one runtime phase.

    The deployment entrypoint requires this variable. The conditional keeps
    source-tree test/dev runs possible while making a configured container
    reject cross-phase routing before an Agent is constructed.
    """
    configured = (os.getenv("XIAOCHUANG_TOOL_PROFILE") or "").strip()
    if configured and configured != tool_profile:
        raise ValueError(
            "Xiaochuang runtime container tool profile does not match this run"
        )


def _xiaochuang_runtime_agent_kwargs() -> Dict[str, Any]:
    """Resolve runtime model config for a Xiaochuang managed run."""
    from gateway.run import _resolve_runtime_agent_kwargs

    return _resolve_runtime_agent_kwargs()


def _validate_xiaochuang_model_gateway_runtime(runtime_kwargs: Dict[str, Any]) -> None:
    """Fail closed unless model calls are forced through Xiaochuang Gateway."""
    from gateway.xiaochuang_runtime_context import (
        is_xiaochuang_model_gateway_base_url,
    )
    ...
    if not is_xiaochuang_model_gateway_base_url(base_url):
        raise ValueError(
            "Xiaochuang managed runs require the model base_url to be the "
            "Xiaochuang Model Gateway"
        )
    if not api_key:
        raise ValueError(
            "Xiaochuang managed runs require a Hermes Model Gateway service identity"
        )


def _has_xiaochuang_run_header(headers: Any) -> bool:
    return any(headers.get(name) for name in XIAOCHUANG_RUN_HEADER_NAMES)
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        xiaochuang_backend_base_url: Optional[str] = None,
        xiaochuang_capability_header: Optional[str] = None,
        xiaochuang_capability_token: Optional[str] = None,
        xiaochuang_execution_id: Optional[str] = None,
        xiaochuang_tool_profile: Optional[str] = None,
    ) -> Any:
        """
        ...
        from gateway.run import _resolve_gateway_model, _load_gateway_config, GatewayRunner
        from hermes_cli.tools_config import _get_platform_tools

        runtime_kwargs = _xiaochuang_runtime_agent_kwargs()
        reasoning_config = GatewayRunner._load_reasoning_config()
        model = _resolve_gateway_model()

        user_config = _load_gateway_config()
        is_xiaochuang_managed_run = bool(
            xiaochuang_capability_token or xiaochuang_backend_base_url
        )
        enabled_toolsets = (
            sorted(_xiaochuang_api_server_toolsets())
            if is_xiaochuang_managed_run
            else sorted(_get_platform_tools(user_config, "api_server"))
        )
        allowed_xiaochuang_tools = frozenset()
        if is_xiaochuang_managed_run:
            _validate_xiaochuang_drama_toolsets(set(enabled_toolsets))
            _validate_xiaochuang_model_gateway_runtime(runtime_kwargs)
            allowed_xiaochuang_tools = _xiaochuang_allowed_tool_names(
                xiaochuang_tool_profile
            )
            if not allowed_xiaochuang_tools:
                raise ValueError("Invalid Xiaochuang tool profile")

        # Xiaochuang workflows are terminated by explicit completion, cancel,
        # revocation, or Provider/network failures. Do not impose a product
        # batch/episode/turn ceiling on a managed run.
        max_iterations = (
            sys.maxsize
            if xiaochuang_capability_token or xiaochuang_backend_base_url
        ...
        try:
            ...
            if is_xiaochuang_model_gateway_base_url(
                runtime_kwargs.get("base_url", "")
            ):
                # A dedicated Xiaochuang pool must never escape to a locally
                # configured fallback Provider where a user key could exist.
                fallback_model = []
        except ImportError:
            pass
        ...
        agent = AIAgent(
            ...
            **runtime_kwargs,
            ...
            # A managed Xiaochuang run is a sealed business execution, not an
            # interactive Hermes session. Its only prompt context is the
            # release-pinned Skill bundle and Backend-provided instruction.
            skip_context_files=is_xiaochuang_managed_run,
            load_soul_identity=False,
            skip_memory=is_xiaochuang_managed_run,
        )
        if allowed_xiaochuang_tools:
            agent.tools = [
                tool
                for tool in (agent.tools or [])
                if tool.get("function", {}).get("name") in allowed_xiaochuang_tools
            ]
            ...
            # Tool-search schemas are cached from the same broad toolset. A
            # managed run must not expose another stage's tool through it.
            agent._tool_search_scope_cache = None
        agent.xiaochuang_backend_base_url = xiaochuang_backend_base_url or ""
        agent.xiaochuang_capability_header = (
            xiaochuang_capability_header or "X-Xiaochuang-MCP-Capability"
        )
        agent.xiaochuang_capability_token = xiaochuang_capability_token or ""
        agent.xiaochuang_execution_id = xiaochuang_execution_id or ""
        agent.xiaochuang_tool_profile = xiaochuang_tool_profile or ""
        return agent
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            config = load_config()
            configured_tool_profile = (
                os.getenv("XIAOCHUANG_TOOL_PROFILE") or ""
            ).strip()
            profile_allowed_tools = sorted(
                _xiaochuang_allowed_tool_names(configured_tool_profile)
            ) if configured_tool_profile else []
            enabled_toolsets = (
                sorted(_xiaochuang_api_server_toolsets())
                if configured_tool_profile
                else _get_platform_tools(
                    config,
                    "api_server",
            ...
            for name, label, desc in _get_effective_configurable_toolsets():
                try:
                    ...
                    tools = []
                if name == XIAOCHUANG_DRAMA_TOOLSET and profile_allowed_tools:
                    tools = profile_allowed_tools
            ...
            for name in sorted(set(enabled_toolsets) - listed_toolsets):
                try:
                    tools = sorted(set(resolve_toolset(name)))
                except Exception:
                    tools = []
                ...
                data.append({
                    "name": name,
                    "label": name,
                    "description": "deployment-only toolset",
                    "enabled": True,
                    "configured": True,
                    "tools": tools,
                })
        ...
        return web.json_response({
            "object": "list",
            "platform": "api_server",
            "xiaochuang_tool_profile": configured_tool_profile or None,
            "xiaochuang_profile_allowed_tools": profile_allowed_tools,
            "data": data,
        })
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        if not re.fullmatch(r"[A-Za-z0-9-]{1,128}", xiaochuang_capability_header):
            return web.json_response(
                _openai_error("Invalid Xiaochuang capability header name"),
                status=400,
            )
        xiaochuang_capability_token = request.headers.get(
            xiaochuang_capability_header, ""
        ).strip()
        xiaochuang_backend_base_url = (
            request.headers.get("X-Xiaochuang-Backend-Base-Url", "") or ""
        ).strip()
        xiaochuang_execution_id = (
            request.headers.get("X-Xiaochuang-Execution-Id", "") or ""
        ).strip()
        xiaochuang_tool_profile = (
            request.headers.get("X-Xiaochuang-Tool-Profile", "") or ""
        ).strip()
        xiaochuang_skill_manifest = (
            request.headers.get(XIAOCHUANG_SKILL_MANIFEST_HEADER, "") or ""
        ).strip()
        xiaochuang_managed_run = _has_xiaochuang_run_header(
            request.headers
        ) or bool(xiaochuang_capability_token)
        xiaochuang_pinned_skills_prompt = ""
        if xiaochuang_managed_run:
            missing = [
                name
                for name, value in (
                    ("capability_token", xiaochuang_capability_token),
                    ("backend_base_url", xiaochuang_backend_base_url),
                    ("execution_id", xiaochuang_execution_id),
                    ("tool_profile", xiaochuang_tool_profile),
                    ("skill_manifest", xiaochuang_skill_manifest),
                )
                if not value
            ]
            if missing:
                return web.json_response(
                    _openai_error(
                        "Missing Xiaochuang runtime headers: "
                        + ", ".join(missing),
                        code="invalid_xiaochuang_runtime",
                    ),
                    status=400,
                )
            if (
                not xiaochuang_execution_id.isdecimal()
                or int(xiaochuang_execution_id) <= 0
            ):
                return web.json_response(
                    _openai_error(
                        "Invalid Xiaochuang execution id",
                        code="invalid_xiaochuang_runtime",
                    ),
                    status=400,
                )
            if not _xiaochuang_allowed_tool_names(xiaochuang_tool_profile):
                return web.json_response(
                    _openai_error(
                        "Invalid Xiaochuang tool profile",
                        code="invalid_xiaochuang_runtime",
                    ),
                    status=400,
                )
            try:
                _validate_xiaochuang_deployment_tool_profile(
                    xiaochuang_tool_profile
                )
            except ValueError as exc:
                return web.json_response(
                    _openai_error(
                        str(exc),
                        code="xiaochuang_tool_profile_mismatch",
                    ),
                    status=409,
                )
            try:
                _validate_xiaochuang_drama_toolsets(
                    _xiaochuang_api_server_toolsets()
                )
            except ValueError as exc:
                return web.json_response(
                    _openai_error(
                        str(exc),
                        code="xiaochuang_toolset_not_restricted",
                    ),
                    status=409,
                )
            try:
                _validate_xiaochuang_model_gateway_runtime(
                    _xiaochuang_runtime_agent_kwargs()
                )
            except ValueError as exc:
                return web.json_response(
                    _openai_error(
                        str(exc),
                        code="xiaochuang_model_gateway_not_configured",
                    ),
                    status=409,
                )
            except Exception:
            ...
            except XiaochuangSkillBundleError:
                return web.json_response(
                    _openai_error(
                        "Xiaochuang runtime Skill bundle is unavailable or does not match the pinned manifest",
                        code="xiaochuang_skill_bundle_invalid",
                    ),
                    status=409,
                )
        ...
        if xiaochuang_managed_run:
            runtime_instruction = (
                str(instructions).strip() if instructions is not None else ""
            )
        ...
        async def _run_and_close():
            try:
                ...
                xiaochuang_tokens = set_xiaochuang_runtime_context(
                    backend_base_url=xiaochuang_backend_base_url,
                    capability_header=xiaochuang_capability_header,
                    capability_token=xiaochuang_capability_token,
                    execution_id=xiaochuang_execution_id,
                    tool_profile=xiaochuang_tool_profile,
                )
                try:
                    # Agent construction creates the model client, so this must
                    # happen while the per-run ContextVar is active.
                    agent = self._create_agent(
                        ephemeral_system_prompt=ephemeral_system_prompt,
                        session_id=session_id,
                        ...
                        gateway_session_key=gateway_session_key,
                        xiaochuang_backend_base_url=xiaochuang_backend_base_url,
                        xiaochuang_capability_header=xiaochuang_capability_header,
                        xiaochuang_capability_token=xiaochuang_capability_token,
                        xiaochuang_execution_id=xiaochuang_execution_id,
                        xiaochuang_tool_profile=xiaochuang_tool_profile,
                    )
                    # Some provider calls happen in worker threads that do
                    # not inherit ContextVars. Keep the private, per-Agent
                    # state reference so run_agent can bind it only for those
                    # requests; it remains scoped to this execution.
                    agent.xiaochuang_runtime_context_reference = (
                        get_xiaochuang_runtime_context_reference()
                    )
                    run_context = contextvars.copy_context()
                finally:
                    clear_xiaochuang_runtime_context(xiaochuang_tokens)
