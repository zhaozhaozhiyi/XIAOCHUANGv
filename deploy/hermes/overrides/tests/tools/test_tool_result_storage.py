...
class TestMaybePersistToolResult:
    ...
    def test_xiaochuang_business_tools_never_persist_or_truncate(self):
        env = MagicMock()
        content = "x" * 250_000
        registry = MagicMock()
        registry.get_toolset_for_tool.return_value = "xiaochuang-drama"

        with patch("tools.registry.registry", registry):
            result = maybe_persist_tool_result(
                content=content,
                tool_name="get_source_chunk",
                tool_use_id="tc_xiaochuang",
                env=env,
                threshold=1,
            )
...
class TestEnforceTurnBudget:
    ...
    def test_xiaochuang_business_results_are_exempt_from_turn_truncation(self):
        msgs = [
            {
                "role": "tool",
        ...
        with patch("tools.registry.registry", registry):
            enforce_turn_budget(msgs, env=None, config=BudgetConfig(turn_budget=1))
