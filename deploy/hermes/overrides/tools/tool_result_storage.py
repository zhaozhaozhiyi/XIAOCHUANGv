...
_XIAOCHUANG_DRAMA_TOOLSET = "xiaochuang-drama"


def _is_xiaochuang_drama_tool(tool_name: str) -> bool:
    """Keep managed business data out of the generic sandbox/truncation path."""
    try:
        from tools.registry import registry

        return (
            registry.get_toolset_for_tool(tool_name)
            == _XIAOCHUANG_DRAMA_TOOLSET
        )
    except Exception:
        return False
...
def maybe_persist_tool_result(
    ...
    """
    # Xiaochuang managed runs cannot use filesystem/terminal tools to recover
    # a persisted result. Their backend already exposes scoped, Agent-driven
    # source/script segments over MCP, so applying the generic Hermes budget
    # here would silently truncate business data and violate the no app-level
    # output cap contract.
    if _is_xiaochuang_drama_tool(tool_name):
        return content
...
def enforce_turn_budget(
    ...
    for i, msg in enumerate(tool_messages):
        ...
        if (
            ...
            and not _is_xiaochuang_drama_tool(tool_name)
        ):
