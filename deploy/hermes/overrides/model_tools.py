...
# The Xiaochuang business bridge registers its fixed tool catalogue from a
# module-level loop. Keep it explicit here because the generic AST discovery
# intentionally only imports modules with direct registry.register(...) calls.
# Importing a module twice is safe, and this makes the managed runtime's tool
# surface deterministic in a fresh process.
importlib.import_module("tools.xiaochuang_drama_tool")
