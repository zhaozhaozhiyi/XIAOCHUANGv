...
def _is_xiaochuang_model_gateway(base_url: Any) -> bool:
    """Return whether *base_url* targets Xiaochuang's fixed Model Gateway."""
    try:
        ...
        return False
    return is_xiaochuang_model_gateway_base_url(base_url)


def _xiaochuang_model_gateway_headers(base_url: Any) -> Dict[str, str]:
    """Return current-run gateway headers without leaking them elsewhere.

    The context helper intentionally raises when a request targets the Model
    Gateway without a capability. Do not catch that failure here: auxiliary
    work must fail closed instead of silently escaping to another provider.
    """
    try:
        from gateway.xiaochuang_runtime_context import (
        ...
        return {}
    return get_xiaochuang_model_gateway_headers(base_url)


def _xiaochuang_model_gateway_runtime(
    main_runtime: Optional[Dict[str, Any]],
) -> Dict[str, Any]:
    """Return a normalized Model Gateway runtime or an empty dict.

    A Hermes run that uses Xiaochuang's Model Gateway must keep every
    auxiliary call on that same route. Task-local provider settings and
    fallback chains are user-level Hermes configuration; they must never
    override Xiaochuang's per-execution authorization boundary.
    """
    runtime = _normalize_main_runtime(main_runtime)
    if _is_xiaochuang_model_gateway(runtime.get("base_url", "")):
        return runtime
    return {}
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        try:
            ...
            pass
    xiaochuang_headers = _xiaochuang_model_gateway_headers(sync_base_url)
    if xiaochuang_headers:
        headers = dict(async_kwargs.get("default_headers") or {})
        headers.update(xiaochuang_headers)
        async_kwargs["default_headers"] = headers
    return AsyncOpenAI(**async_kwargs), model


def _resolve_xiaochuang_model_gateway_client(
    *,
    model: Optional[str],
    async_mode: bool,
    main_runtime: Optional[Dict[str, Any]],
    ...
    """Create an uncached auxiliary client for the active Xiaochuang run."""
    runtime = _xiaochuang_model_gateway_runtime(main_runtime)
    if not runtime:
        return None, None

    base_url = runtime.get("base_url", "")
    api_key = runtime.get("api_key", "")
    final_model = str(runtime.get("model") or model or "").strip()
    if not api_key:
        raise RuntimeError(
            "Xiaochuang Model Gateway requires the Hermes service credential"
        )
    if not final_model:
        raise RuntimeError(
            "Xiaochuang Model Gateway requires a model selected by the platform"
        )

    # This call validates that the capability and execution context exist.
    # The returned headers are only attached to the fixed Gateway URL.
    headers = _xiaochuang_model_gateway_headers(base_url)
    client = OpenAI(
        api_key=api_key,
...
def resolve_provider_client(
    ...
    gateway_runtime = _xiaochuang_model_gateway_runtime(main_runtime)
    if gateway_runtime:
        # The platform owns provider resolution for a gateway-backed run.
        # Do this before provider normalization and config-derived fallbacks
        # so no auxiliary path can reach Hermes-local credentials.
        return _resolve_xiaochuang_model_gateway_client(
            model=model,
            async_mode=async_mode,
            main_runtime=gateway_runtime,
            is_vision=is_vision,
        )
        ...
        if explicit_base_url:
            ...
            if base_url_host_matches(custom_base, "api.kimi.com"):
                ...
                try:
                    ...
                    pass
            xiaochuang_headers = _xiaochuang_model_gateway_headers(custom_base)
            if xiaochuang_headers:
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    gateway_runtime = _xiaochuang_model_gateway_runtime(main_runtime)
    if gateway_runtime:
        client, final_model = _resolve_xiaochuang_model_gateway_client(
...
def _get_cached_client(
    ...
    if async_mode:
        try:
            ...
            pass
    runtime = _normalize_main_runtime(main_runtime)
    if _is_xiaochuang_model_gateway(
        runtime.get("base_url", "") or base_url or ""
    ):
        # Capability-bearing clients are deliberately never cached. A cache
        # keyed by service credential/base URL would otherwise reuse a prior
        # execution's capability header for a later task.
        return _resolve_xiaochuang_model_gateway_client(
            model=model,
            async_mode=async_mode,
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        is_xiaochuang_gateway = _is_xiaochuang_model_gateway(_base_info)
        ...
        if should_fallback and not is_xiaochuang_gateway and (is_auto or is_capacity_error):
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        is_xiaochuang_gateway = _is_xiaochuang_model_gateway(_client_base)
