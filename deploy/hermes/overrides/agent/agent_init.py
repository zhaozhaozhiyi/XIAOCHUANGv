...
def init_agent(
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        # A Xiaochuang capability is allowed to leave Hermes only on requests
        # to Xiaochuang's fixed Model Gateway. The helper is a no-op for every
        # other Provider and fails closed when the gateway lacks per-run state.
        try:
    ...
    # Xiaochuang Model Gateway intentionally exposes only the OpenAI
    # inference endpoints. It is not an Ollama/local-model endpoint even
    # when reached through an internal address. Do not probe /models or
    # /api/show during Agent construction; use the same conservative context
    # fallback Hermes would use after an unknown-provider probe instead.
    _xiaochuang_model_gateway = False
    try:
        from gateway.xiaochuang_runtime_context import (
            is_xiaochuang_model_gateway_base_url,
        )

        _xiaochuang_model_gateway = is_xiaochuang_model_gateway_base_url(
            agent.base_url
        )
    except ImportError:
        pass
    if _xiaochuang_model_gateway and _config_context_length is None:
        _config_context_length = 256_000

    # Persist for reuse on switch_model / fallback activation. Must come
    # AFTER the custom_providers branch so per-model overrides aren't lost.
    agent._config_context_length = _config_context_length
    agent._xiaochuang_model_gateway = _xiaochuang_model_gateway
    ...
    if (
        ...
        and agent.base_url
        and not agent._xiaochuang_model_gateway
