...
import {
  downloadFile,
  normalizeImageReferenceForAdapter,
} from "../images/images.storage";
import {
  appendDramaStyleHint,
  parseConfigModelList,
...
import {
  ...
  type ContinuityRunInstruction,
} from "./continuity-production.service";
...
type EnqueueVideoOptions = {
  beforeQueue?: (videoGenerationId: number) => Promise<void>;
};

type BuildVideoRequestOptions = {
  isContinuityRun?: boolean;
};
...
function resolveStoryboardStatusAfterVideoSuccess(
  storyboard: typeof storyboards.$inferSelect | null | undefined,
) {
  return storyboard?.composedVideoUrl ? "compose_completed" : "video_completed";
}

function resolveStoryboardStatusAfterVideoFailure(
  storyboard: typeof storyboards.$inferSelect | null | undefined,
) {
...
export class VideosService {
  constructor(
    ...
    @Inject(VideosTasksService)
    private readonly videosTasksService: VideosTasksService,
  ...
  async listOwnedVideoGenerations(
    userId: number,
    query: { dramaId?: number; storyboardId?: number },
  ) {
    ...
    if (query.storyboardId)
      rows = rows.filter((row) => row.storyboardId === query.storyboardId);
    if (query.dramaId)
      rows = rows.filter((row) => row.dramaId === query.dramaId);
    return rows;
  }
  ...
  private async assertDirectVideoGenerationAllowed(
    storyboard: typeof storyboards.$inferSelect,
    userId: number,
  ) {
    if (storyboard.storyboardSetId == null) return;

    const [activeSet] = await this.databaseService.db
      .select({ id: storyboardSets.id })
      .from(storyboardSets)
      .where(
        and(
          eq(storyboardSets.id, storyboard.storyboardSetId),
          eq(storyboardSets.userId, userId),
          eq(storyboardSets.episodeId, storyboard.episodeId),
    ...
    const [boundary] = await this.databaseService.db
      ...
      .where(
        and(
          ...
          eq(storyboardBoundaries.episodeId, storyboard.episodeId),
          eq(
            storyboardBoundaries.sourceStoryboardSetId,
            storyboard.storyboardSetId,
          ),
          or(
            eq(storyboardBoundaries.fromStoryboardId, storyboard.id),
            eq(storyboardBoundaries.toStoryboardId, storyboard.id),
          ),
  ...
  async buildVideoRequest(
    body: Record<string, unknown>,
    userId: number,
    options: BuildVideoRequestOptions = {},
  ) {
    let prompt = String(body.prompt || "").trim();
    let configId =
      typeof body.config_id === "number"
        ? body.config_id
        : typeof body.config_id === "string"
          ? Number(body.config_id)
          : undefined;
    let dramaId =
      typeof body.drama_id === "number"
        ? body.drama_id
        : typeof body.drama_id === "string"
          ? Number(body.drama_id)
          : undefined;
    let referenceMode = normalizeReferenceMode(body.reference_mode);
    let imageUrl =
      typeof body.image_url === "string" ? body.image_url.trim() : undefined;
    let firstFrameUrl =
      typeof body.first_frame_url === "string"
        ? body.first_frame_url.trim()
        : undefined;
    let lastFrameUrl =
      typeof body.last_frame_url === "string"
        ? body.last_frame_url.trim()
        : undefined;
    ...
    if (body.storyboard_id) {
      const storyboard = await requireOwnedStoryboard(
        this.databaseService,
        Number(body.storyboard_id),
        userId,
      );
      if (!options.isContinuityRun) {
        await this.assertDirectVideoGenerationAllowed(storyboard, userId);
      }
      ...
      if (episode?.videoConfigId != null) {
        ...
        if (drama)
          configId =
            resolveProjectConfigId(drama.metadata, "video") ?? undefined;
      }
      if (episode?.dramaId != null && dramaId == null)
        dramaId = episode.dramaId;
      if (!prompt) prompt = resolveStoryboardVideoPrompt(storyboard);
      if (!imageUrl && storyboard.composedImage)
        imageUrl = storyboard.composedImage;
      if (!firstFrameUrl && storyboard.firstFrameImage)
        firstFrameUrl = storyboard.firstFrameImage;
      if (!lastFrameUrl && storyboard.lastFrameImage)
        lastFrameUrl = storyboard.lastFrameImage;
      if (!referenceImageUrls?.length)
        referenceImageUrls = parseStringArray(storyboard.referenceImages);
      if (
        duration == null &&
        typeof storyboard.duration === "number" &&
  ...
  async enqueueVideoGeneration(
    ...
    options?: EnqueueVideoOptions,
  ) {
    const ts = now();
    const configRow = await this.aiConfigResolver.resolveConfigRow(
      "video",
      params.configId,
      params.userId,
    );
    const config =
      resolvedConfig ||
      (await this.aiConfigResolver.resolveConfig(
        "video",
        params.configId,
        params.userId,
      ));
    ...
    const taskId = await this.videosTasksService.syncTaskForVideoGeneration(
      created.id,
      {
    ...
    if (options?.beforeQueue) {
      await options.beforeQueue(created.id);
    }
    if (taskId != null) {
  ...
  async enqueueContinuityRunItem(instruction: ContinuityRunInstruction) {
    if (!this.continuityProductionService) {
      throw new Error("continuity_production_service_unavailable");
    }

    try {
      const params = await this.buildVideoRequest(
        {
          drama_id: instruction.dramaId,
          storyboard_id: instruction.storyboardId,
          first_frame_url: instruction.startAnchorUrl,
          last_frame_url: instruction.plannedEndAnchorUrl ?? undefined,
          reference_image_urls: instruction.referenceImageUrls,
        },
        instruction.userId,
        { isContinuityRun: true },
      );
      const generationId = await this.enqueueVideoGeneration({
        ...params,
        taskPayload: {
          ...(params.taskPayload ?? {}),
          continuity_run_id: instruction.runId,
          continuity_run_item_id: instruction.runItemId,
          continuity_start_anchor_url: instruction.startAnchorUrl,
        },
      }, undefined, {
        beforeQueue: async (videoGenerationId) => {
          await this.continuityProductionService!.bindVideoGeneration(
            instruction.runItemId,
            videoGenerationId,
          );
        },
      });
      ...
      await this.continuityProductionService.markEnqueueFailed(
        instruction.runItemId,
        error,
      );
      throw error;
  ...
  async retryContinuityVideoGeneration(input: {
    ...
    await this.continuityProductionService.retryVideoGeneration(input);
  }
  ...
  async cancelVideoGeneration(
    ...
    const [record] = await this.databaseService.db
      ...
      .from(videoGenerations)
      .where(eq(videoGenerations.id, id));
    if (!record || isTerminalVideoGenerationStatus(record.status)) {
      return false;
    }
    ...
    await this.notifyContinuityVideoCanceled(id, reason);
    return true;
  }

  async failVideoGeneration(id: number, error: unknown) {
    const [record] = await this.databaseService.db
      .select()
      .from(videoGenerations)
      .where(eq(videoGenerations.id, id));
    ...
    const message =
      error instanceof Error ? error.message : String(error || "Video generation failed");
    await this.databaseService.db
      .update(videoGenerations)
      .set({
        status: "failed",
    ...
    await this.videosTasksService.syncTaskForVideoGeneration(id);
    await this.restoreStoryboardVideoStatus(record.storyboardId);
    await this.notifyContinuityVideoFailure(id, message);
    return true;
  }
  ...
  private async notifyContinuityVideoFailure(id: number, error: unknown) {
    try {
      await this.continuityProductionService?.markVideoGenerationFailed(
        id,
        error,
      );
    } catch (continuityError) {
      console.error(
        "[VideosService] Failed to update continuity production after video failure",
        continuityError,
      );
    }
  }

  private async notifyContinuityVideoCanceled(id: number, reason: unknown) {
    try {
      await this.continuityProductionService?.markVideoGenerationCanceled(
        id,
        reason,
      );
    } catch (continuityError) {
      console.error(
        "[VideosService] Failed to update continuity production after video cancellation",
        continuityError,
      );
    }
  }

  private async advanceContinuityVideoRun(id: number, videoUrl: string) {
    if (!this.continuityProductionService) return;
    try {
      const nextInstructions =
        await this.continuityProductionService.completeVideoGeneration(
          id,
          videoUrl,
        );
      for (const instruction of nextInstructions) {
        await this.enqueueContinuityRunItem(instruction);
      }
    } catch (error) {
      console.error(
        "[VideosService] Failed to advance continuity production after video completion",
        error,
      );
    }
  }
  ...
  private async processVideoGenerationWithConfig(id: number, config: AIConfig) {
    ...
    try {
      ...
      const resolvedImageUrl = await this.normalizeVideoReferenceUrl(
        record.imageUrl,
      );
      const resolvedFirstFrameUrl = await this.normalizeVideoReferenceUrl(
        record.firstFrameUrl,
      );
      const resolvedLastFrameUrl = await this.normalizeVideoReferenceUrl(
        record.lastFrameUrl,
      );
      const resolvedReferenceImageUrls = await this.normalizeVideoReferenceUrls(
      ...
      const { isAsync, taskId, videoUrl } =
        adapter.parseGenerateResponse(result);

      if (!isAsync && videoUrl) {
        await this.handleVideoComplete(
          id,
          videoUrl,
          record.duration,
          record.storyboardId,
      ...
      const message =
        ...
        (record.imageUrl ||
          ...
          record.lastFrameUrl ||
          record.referenceImageUrls)
          ? `continuity_reference_rejected: ${rawMessage}`
          : rawMessage;
      ...
      await this.databaseService.db
        ...
        .where(eq(videoGenerations.id, id));
      await this.videosTasksService.syncTaskForVideoGeneration(id);
      await this.restoreStoryboardVideoStatus(
        latestRecord?.storyboardId ?? record?.storyboardId,
      );
      await this.notifyContinuityVideoFailure(id, message);
    }
  }

  private async normalizeVideoReferenceUrl(
  ...
  private async normalizeVideoReferenceUrls(
    ...
    const normalized = await Promise.all(
      ...
      ).map((item) => this.normalizeVideoReferenceUrl(item)),
    );
  ...
  private async pollVideoTask(
    ...
    for (let i = 0; i < 300; i += 1) {
      ...
      try {
        const { url, method, headers } = adapter.buildPollRequest(
          config,
          taskId,
        );
        const response = await fetch(url, {
        ...
        if (i === 299) {
          ...
          await this.notifyContinuityVideoFailure(id, `Timeout: ${message}`);
          return;
        }
      }
    }
  ...
  private async handleVideoComplete(
    ...
    //    避免并发重复 webhook 造成重复下载 + 重复写 storyboard。允许回收超过 5 分钟的陈旧 'finalizing'。
    ...
    const [claimed] = await this.databaseService.db
      ...
      .where(
        and(
          eq(videoGenerations.id, id),
          not(
            inArray(videoGenerations.status, [
              "completed",
              "failed",
              "canceled",
              "cancelled",
      ...
      .returning({
        ...
        storyboardId: videoGenerations.storyboardId,
      });
    ...
    try {
      ...
      await this.databaseService.db
        ...
        .where(
          and(
            eq(videoGenerations.id, id),
            eq(videoGenerations.status, "finalizing"),
          ),
        );
      throw downloadError;
    }

    // 3. 从 'finalizing' 推进到 'completed'
    await this.databaseService.db
      .update(videoGenerations)
      .set({
    ...
    await this.advanceContinuityVideoRun(id, storedFile.url);
  }
  ...
  async handleViduWebhook(body: Record<string, unknown>) {
    ...
    if (state === "success" && videoUrl) {
      try {
        ...
        await this.restoreStoryboardVideoStatus(record.storyboardId);
        await this.notifyContinuityVideoFailure(
          record.id,
          `Webhook download failed: ${message}`,
        );
