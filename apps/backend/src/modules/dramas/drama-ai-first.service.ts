...
import { aiRuns, dramaSourceChunks, dramaSources, dramas, episodes, tasks } from '../../db/schema'
...
type EpisodeBlueprintContext = {
  ...
  health: SourceHealth
  analysis: SourceAnalysisPayload
  selectedBrief: AdaptationBriefPayload
...
function normalizeSourceType(value: SourceType | string | null | undefined): SourceType {
  return value === 'upload' || value === 'writing_project' ? value : 'paste'
}

const EPISODE_STALE_SUFFIX_PATTERN = /(?:_(?:source|analysis|strategy|blueprint)_stale)+$/
...
export function markEpisodeGenerationModeStale(
  ...
  const base = normalized.replace(EPISODE_STALE_SUFFIX_PATTERN, '') || (hasScript ? 'script' : 'blueprint')
  return `${base}${staleSuffix}`
}

export function markEpisodeGenerationModeSourceStale(mode: string | null | undefined, hasScript: boolean) {
  return markEpisodeGenerationModeStale(mode, hasScript, 'source')
}

async function markEpisodeOutputsStale(
  db: EpisodeStaleDb,
  input: { userId: number; dramaId: number; reason: EpisodeStaleReason; timestamp: Date },
) {
  const episodeRows = await db
    ...
    .from(episodes)
    .where(and(eq(episodes.dramaId, input.dramaId), eq(episodes.userId, input.userId), isNull(episodes.deletedAt)))
  ...
  for (const episode of episodeRows) {
    ...
    await db
      .update(episodes)
      .set({
        generationMode: markEpisodeGenerationModeStale(episode.generationMode, hasScript, input.reason),
        updatedAt: input.timestamp,
      })
      .where(and(eq(episodes.id, episode.id), eq(episodes.userId, input.userId)))
  }
}
...
function isChapterHeading(line: string) {
  ...
  return /^(?:#{1,6}\s*)?(?:第\s*[0-9０-９一二三四五六七八九十百千万零〇两俩]+\s*[章节集卷回幕]|chapter\s*\d+|episode\s*\d+|ep\.?\s*\d+)/i.test(normalized)
}
...
function buildHealth(content: string): SourceHealth {
  ...
  const hasBlockedIssue = anomalies.some((anomaly) => anomaly.severity === 'blocked')
  const hasWarningIssue = anomalies.some((anomaly) => anomaly.severity === 'warning')
  ...
  return {
    ...
    chunk_count: overContextLimit ? Math.max(1, Math.ceil(estimatedTokens / DEFAULT_CHUNK_TOKENS)) : 0,
...
function buildChunks(content: string, sourceId: number, health: SourceHealth) {
  ...
  const maxChars = Math.max(1_000, Math.floor(DEFAULT_CHUNK_TOKENS / TOKEN_PER_SOURCE_CHAR))
  const chunks: Array<{
    sourceId: number
    chunkNo: number
    title: string
  ...
  while (start < content.length) {
    ...
    const nextBreak = content.lastIndexOf('\n', hardEnd)
    const end = nextBreak > start + Math.floor(maxChars * 0.6) ? nextBreak : hardEnd
    ...
    chunks.push({
      ...
      sourceTrace: JSON.stringify([{ source_id: sourceId, content_start: start, content_end: end }]),
    })
    start = end
    while (content[start] === '\n') start += 1
  }
...
function pickLikelyCharacterNames(content: string) {
  const denylist = new Set(['旁白', '字幕', '镜头', '系统', '众人', '大家', '我们', '他们'])
  const counts = new Map<string, number>()

  for (const match of content.matchAll(/([\u4e00-\u9fa5]{1,4})(?:说|问|喊|笑|看|推开|走进|回头|皱眉|沉默|冷笑|握住)/g)) {
    const name = String(match[1] || '').trim()
    if (!name || denylist.has(name)) continue
    counts.set(name, (counts.get(name) || 0) + 1)
  }
  for (const match of content.matchAll(/(?:主角|女主|男主|少女|少年|总裁|医生|律师|皇帝|王爷|夫人|小姐)[：:，,]?([\u4e00-\u9fa5]{1,4})/g)) {
    const name = String(match[1] || '').trim()
    if (!name || denylist.has(name)) continue
...
function pickSceneNames(chapterIndex: NonNullable<SourceHealth['chapter_index']>, content: string) {
  const names = new Set<string>()
  for (const match of content.matchAll(/(?:在|到|回到|来到)([\u4e00-\u9fa5]{2,8})(?:里|中|前|后|门口|大厅|房间|院子|街头|医院|公司|学校|宫殿|客厅|书房)/g)) {
    const name = String(match[1] || '').trim()
...
function createSourceTrace(sourceId: number, health: SourceHealth, index = 0) {
  const chapter = health.chapter_index?.[index % Math.max(health.chapter_index.length, 1)]
  return [{
    source_id: sourceId,
...
function readSourceHealth(aiFirst: Record<string, unknown>): SourceHealth | null {
  const health = toRecord(aiFirst.source_health)
  if (!health.status) return null
  return health as SourceHealth
}

function readSourceAnalysis(aiFirst: Record<string, unknown>): SourceAnalysisPayload | null {
...
export class DramaAiFirstService {
  constructor(
    ...
    @Optional() @Inject(DramaAgentService) private readonly dramaAgentService?: DramaAgentService,
    @Optional() @Inject(TaskQueueService) private readonly taskQueueService?: TaskQueueService,
  ) {}
  ...
  private shouldUseLocalRuleFallback() {
    return readBooleanEnv('DRAMA_AI_FIRST_LOCAL_RULE_FALLBACK', process.env.NODE_ENV === 'test')
  }
  ...
  async saveSource(input: SaveSourceInput) {
    ...
    await this.databaseService.db.transaction(async (tx) => {
      const [drama] = await tx
        .select()
        .from(dramas)
        .where(and(eq(dramas.id, input.dramaId), eq(dramas.userId, input.userId), isNull(dramas.deletedAt)))
      ...
      const nextMetadata: Record<string, unknown> = {
        ...
        ai_first: {
          ...
          source_analysis: sourceChanged ? null : aiFirst.source_analysis ?? null,
          adaptation_briefs: sourceChanged ? [] : Array.isArray(aiFirst.adaptation_briefs) ? aiFirst.adaptation_briefs : [],
          selected_brief_id: sourceChanged ? '' : toStringValue(aiFirst.selected_brief_id),
      ...
      await tx
        ...
        .where(and(eq(dramas.id, input.dramaId), eq(dramas.userId, input.userId)))
  ...
  async healthCheck(input: HealthCheckInput) {
    ...
    const [source] = await this.databaseService.db
      .select()
      .from(dramaSources)
      .where(and(eq(dramaSources.dramaId, input.dramaId), eq(dramaSources.userId, input.userId), isNull(dramaSources.deletedAt)))
  ...
  async analyzeSource(input: { userId: number; dramaId: number }) {
    ...
    if (this.shouldQueueSourceAnalysis(context.health)) {
  ...
  async executeSourceAnalysisTask(input: { taskId: number; userId: number; dramaId: number; sourceId: number }) {
  ...
  private async analyzeSourceNow(input: { userId: number; dramaId: number; sourceId?: number; taskId?: number }) {
    const { drama, metadata, aiFirst, health, source } = await this.loadSourceAnalysisContext(input)
    ...
    if (input.taskId) {
      await this.updateSourceAnalysisTask(input.taskId, {
        ...
        resultSummaryJson: JSON.stringify({
          phase: health.over_context_limit ? 'chunk_analysis' : 'source_analysis',
          source_id: source.id,
    ...
    if (useRemoteAgent) {
      ...
      analysis = this.buildLocalSourceAnalysis({ dramaTitle: drama.title, sourceId: source.id, content: source.content, health, aiRunId: run.id })
    } else {
      throw this.buildAiFirstAgentRequiredError('source_analysis')
    }
        ...
        .set({
          metadata: JSON.stringify({
            ...
            ai_first: {
              ...
              adaptation_briefs: [],
              selected_brief_id: '',
              source_analysis_task_id: input.taskId || toNumberValue(aiFirst.source_analysis_task_id) || null,
              source_analysis_task_status: input.taskId ? 'completed' : toStringValue(aiFirst.source_analysis_task_status),
    ...
    if (health.over_context_limit && analysis.generation_mode !== REMOTE_AGENT_MODE) {
      await this.databaseService.db
        ...
        .where(and(eq(dramaSourceChunks.sourceId, source.id), eq(dramaSourceChunks.userId, input.userId)))
    }
  ...
  private async loadSourceAnalysisContext(input: {
    ...
    if (health.status === 'blocked') throw new BadRequestException('source_blocked')

    const metadataSourceId = Number(aiFirst.source_id)
    const sourceId = Number.isInteger(input.sourceId) && Number(input.sourceId) > 0
    ...
    const [source] = sourceId
      ? await this.databaseService.db
        .select()
        .from(dramaSources)
        .where(and(eq(dramaSources.id, sourceId), eq(dramaSources.dramaId, input.dramaId), eq(dramaSources.userId, input.userId), isNull(dramaSources.deletedAt)))
        .limit(1)
      : await this.databaseService.db
        .select()
        .from(dramaSources)
  ...
  private shouldQueueSourceAnalysis(health: SourceHealth) {
    return health.over_context_limit && health.recommended_mode === 'long_source_async' && Boolean(this.taskQueueService)
  }

  private async createOrReuseSourceAnalysisTask(context: SourceAnalysisContext) {
    const timestamp = new Date()
    const title = compactText(`AI-first 源稿理解：${context.source.title || context.drama.title}`, 120)
    const payload = {
    ...
    if (existing) {
      const [updated] = await this.databaseService.db
        ...
        .set({
          ...
          aiConfigId: null,
          providerTaskId: null,
          attemptCount: existing.status === 'failed' || existing.status === 'dead_letter' ? 0 : existing.attemptCount,
          lockedBy: null,
          lockedAt: null,
          lockExpiresAt: null,
          payloadJson: JSON.stringify(payload),
  ...
  private async markSourceAnalysisTaskQueuedInMetadata(context: SourceAnalysisContext, taskId: number) {
    ...
    await this.databaseService.db
      ...
      .where(and(eq(dramas.id, context.drama.id), eq(dramas.userId, context.source.userId)))
  }

  private async updateSourceAnalysisTask(taskId: number, values: Partial<typeof tasks.$inferInsert>) {
    await this.databaseService.db
      .update(tasks)
      .set({
        ...values,
  ...
  private async completeSourceAnalysisTask(taskId: number, summary: Record<string, unknown>) {
    const timestamp = new Date()
    await this.databaseService.db
      .update(tasks)
      .set({
  ...
  async failSourceAnalysisTask(taskId: number, error: unknown) {
    ...
    await this.databaseService.db
      ...
      .set({
        status: 'failed',
        errorKind: message.toLowerCase().includes('outdated') ? 'outdated' : 'provider',
        errorMessage: compactText(message, 500),
        errorDetailsJson: JSON.stringify({
          error_kind: message.toLowerCase().includes('outdated') ? 'outdated' : 'provider',
          raw_error: message,
        }),
        completedAt: timestamp,
        lockedBy: null,
  ...
  async executeAdaptationBriefsTask(input: GenerateBriefInput & { taskId: number }) {
  ...
  private async generateAdaptationBriefsNow(input: GenerateBriefInput & { taskId?: number }) {
    const { drama, metadata, aiFirst, analysis, health, count, targetEpisodeCount, episodeDuration, styleDirection } =
    ...
    if (input.taskId) {
      await this.updateAdaptationBriefsTask(input.taskId, {
        ...
        resultSummaryJson: JSON.stringify({
          ...
          drama_id: input.dramaId,
          target_episode_count: targetEpisodeCount || drama.totalEpisodes || null,
          brief_count: count,
        }),
      })
    }
    ...
    if (useRemoteAgent) {
      try {
        const agent = await this.dramaAgentService!.generateAdaptationBriefs({
          ...
          health,
          count,
          targetEpisodeCount: targetEpisodeCount || drama.totalEpisodes || undefined,
          episodeDuration,
        ...
        .set({
          metadata: JSON.stringify({
            ...
            ai_first: {
              ...
              selected_brief_id: '',
              ai_first_stage: 'brief_pending',
              brief_task_id: input.taskId || toNumberValue(aiFirst.brief_task_id) || null,
              brief_task_status: input.taskId ? 'completed' : toStringValue(aiFirst.brief_task_status),
              ai_first_updated_at: generatedAt,
            },
          }),
          updatedAt,
  ...
  private async loadAdaptationBriefContext(input: GenerateBriefInput): Promise<AdaptationBriefContext> {
    const [drama] = await this.databaseService.db
      .select()
      .from(dramas)
      .where(and(eq(dramas.id, input.dramaId), eq(dramas.userId, input.userId), isNull(dramas.deletedAt)))
    ...
    const health = readSourceHealth(aiFirst)
    if (!health || health.status === 'blocked') throw new BadRequestException('usable_source_required')
  ...
  private async createOrReuseAdaptationBriefsTask(context: AdaptationBriefContext) {
    const timestamp = new Date()
    ...
    const payload = {
      ...
      count: context.count,
      target_episode_count: context.targetEpisodeCount || context.drama.totalEpisodes || null,
  ...
  private async markAdaptationBriefsTaskQueued(context: AdaptationBriefContext, taskId: number) {
    const timestamp = new Date()
    await this.databaseService.db
      .update(dramas)
      .set({
      ...
      .where(and(eq(dramas.id, context.drama.id), eq(dramas.userId, Number(context.drama.userId))))
  }

  private async updateAdaptationBriefsTask(taskId: number, values: Partial<typeof tasks.$inferInsert>) {
    await this.databaseService.db
      .update(tasks)
      .set({
        ...values,
  ...
  private async completeAdaptationBriefsTask(taskId: number, summary: Record<string, unknown>) {
    const timestamp = new Date()
    const [task] = await this.databaseService.db
      .select()
      .from(tasks)
  ...
  async failAdaptationBriefsTask(taskId: number, error: unknown) {
    ...
    if (task?.dramaId && task.userId) {
      const [drama] = await this.databaseService.db
        .select()
        .from(dramas)
        .where(and(eq(dramas.id, task.dramaId), eq(dramas.userId, task.userId), isNull(dramas.deletedAt)))
        .limit(1)
      if (drama) {
        const { metadata, aiFirst } = readAiFirst(drama.metadata)
        await this.databaseService.db
          ...
          .set({
            metadata: JSON.stringify({
              ...
              ai_first: {
                ...aiFirst,
                ai_first_stage: toStringValue(aiFirst.ai_first_stage) || 'brief_pending',
                brief_task_id: taskId,
  ...
  async selectAdaptationBrief(input: { userId: number; dramaId: number; briefId: string }) {
    this.assertAiFirstEnabled()
    const [drama] = await this.databaseService.db
      .select()
      .from(dramas)
    ...
    if (!briefs.some((brief) => brief.id === input.briefId)) throw new NotFoundException('adaptation_brief_not_found')
    const nowIso = new Date().toISOString()

    const updatedAt = new Date()
    const selectedBriefChanged = toStringValue(aiFirst.selected_brief_id) !== input.briefId
    await this.databaseService.db.transaction(async (tx) => {
      await tx
        .update(dramas)
        .set({
  ...
  async executeEpisodeBlueprintsTask(input: GenerateBlueprintInput & { taskId: number }) {
  ...
  private async generateEpisodeBlueprintsNow(input: GenerateBlueprintInput & { taskId?: number }) {
    const context = await this.loadEpisodeBlueprintContext(input)
    const { drama, metadata, aiFirst, health, analysis, selectedBrief, existingEpisodes, sourceId } = context
    ...
    if (input.taskId) {
      await this.updateEpisodeBlueprintsTask(input.taskId, {
        ...
        resultSummaryJson: JSON.stringify({
          ...
          drama_id: input.dramaId,
          selected_brief_id: selectedBrief.id,
          target_episodes: selectedBrief.target_episode_count,
        }),
      })
    }
    ...
    if (useRemoteAgent) {
      try {
        ...
        const run = await this.recordRemoteRun({
          ...
          userMessage: `Generate episode blueprints for brief ${selectedBrief.id}`,
          remoteRunId: agent.remoteRunId,
          usage: agent.usage,
        ...
        blueprints = agent.blueprints.map((blueprint) => ({
          ...blueprint,
          brief_id: blueprint.brief_id || selectedBrief.id,
          ai_run_id: run.id,
          remote_run_id: agent.remoteRunId,
          generated_at: generatedAt,
          generation_mode: REMOTE_AGENT_MODE,
    ...
    await this.databaseService.db.transaction(async (tx) => {
      for (const blueprint of blueprints) {
        const existing = existingEpisodes.find((episode) => episode.episodeNumber === blueprint.episode_number)
        if (existing) {
          if (existing.scriptContent?.trim()) continue
          await tx
            .update(episodes)
            .set({
              ...
              generationMode: blueprint.generation_mode === REMOTE_AGENT_MODE ? 'remote_agent_blueprint' : 'local_rule_blueprint',
              failureReason: null,
              status: 'blueprint',
              updatedAt: ts,
            })
            .where(and(eq(episodes.id, existing.id), eq(episodes.userId, input.userId)))
        } else {
        ...
        .set({
          ...
          metadata: JSON.stringify({
            ...
            ai_first: {
              ...
              blueprint_task_id: input.taskId || toNumberValue(aiFirst.blueprint_task_id) || null,
              blueprint_task_status: input.taskId ? 'completed' : toStringValue(aiFirst.blueprint_task_status),
              ai_first_updated_at: generatedAt,
            },
          }),
          updatedAt: ts,
  ...
  async regenerateEpisodeBlueprint(input: RegenerateEpisodeBlueprintInput) {
    ...
    const [episode] = await this.databaseService.db
      .select()
      .from(episodes)
      .where(and(eq(episodes.id, input.episodeId), eq(episodes.userId, input.userId), isNull(episodes.deletedAt)))
      .limit(1)
    ...
    const targetEpisode = context.existingEpisodes.find((item) => item.id === episode.id)
    ...
    if (useRemoteAgent) {
      try {
        const agent = await this.dramaAgentService!.generateSingleEpisodeBlueprint({
          userId: input.userId,
          dramaId: episode.dramaId,
        ...
        blueprint = {
          ...
          brief_id: agent.blueprint.brief_id || context.selectedBrief.id,
          ai_run_id: run.id,
          remote_run_id: agent.remoteRunId,
          generated_at: generatedAt,
          generation_mode: REMOTE_AGENT_MODE,
      ...
      const blueprints = this.buildLocalEpisodeBlueprints({
        ...
        analysis: context.analysis,
        brief: {
          ...context.selectedBrief,
          target_episode_count: Math.max(context.selectedBrief.target_episode_count || 1, episode.episodeNumber),
        },
        aiRunId: run.id,
        generatedAt,
      })
      const selectedBlueprint = blueprints.find((item) => item.episode_number === episode.episodeNumber)
      if (!selectedBlueprint) throw new BadRequestException('episode_blueprint_required')
    ...
    await this.databaseService.db.transaction(async (tx) => {
      ...
      await tx
        .update(dramas)
        .set({
          totalEpisodes: Math.max(context.drama.totalEpisodes || 0, episode.episodeNumber),
          metadata: JSON.stringify({
            ...context.metadata,
            ai_first: {
              ...context.aiFirst,
        ...
        .where(and(eq(dramas.id, episode.dramaId), eq(dramas.userId, input.userId)))
    })
  ...
  private async loadEpisodeBlueprintContext(input: GenerateBlueprintInput): Promise<EpisodeBlueprintContext> {
    const [drama] = await this.databaseService.db
      .select()
      .from(dramas)
      .where(and(eq(dramas.id, input.dramaId), eq(dramas.userId, input.userId), isNull(dramas.deletedAt)))
    ...
    if (!health || !analysis) throw new BadRequestException('source_analysis_required')
    if (!selectedBrief) throw new BadRequestException('selected_brief_required')
    ...
    if (existingEpisodes.some((episode) => episode.scriptContent?.trim()) && !input.replaceWithoutScript) {
  ...
  private async createOrReuseEpisodeBlueprintsTask(context: EpisodeBlueprintContext) {
    const timestamp = new Date()
    ...
    const payload = {
      ...
      content_hash: toStringValue(context.aiFirst.content_hash) || null,
      selected_brief_id: context.selectedBrief.id,
      target_episode_count: context.selectedBrief.target_episode_count,
  ...
  private async markEpisodeBlueprintsTaskQueued(context: EpisodeBlueprintContext, taskId: number) {
    const timestamp = new Date()
    await this.databaseService.db
      .update(dramas)
      .set({
  ...
  private async updateEpisodeBlueprintsTask(taskId: number, values: Partial<typeof tasks.$inferInsert>) {
    await this.databaseService.db
      .update(tasks)
      .set({
        ...values,
  ...
  private async completeEpisodeBlueprintsTask(taskId: number, summary: Record<string, unknown>) {
    const timestamp = new Date()
    const [task] = await this.databaseService.db
      .select()
      .from(tasks)
  ...
  async failEpisodeBlueprintsTask(taskId: number, error: unknown) {
    ...
    if (task?.dramaId && task.userId) {
      ...
      if (drama) {
        ...
        const fallbackStage = toStringValue(aiFirst.selected_brief_id) ? 'brief_selected' : 'brief_pending'
        await this.databaseService.db
          .update(dramas)
          .set({
            metadata: JSON.stringify({
              ...
              ai_first: {
                ...aiFirst,
                ai_first_stage: currentStage === 'blueprint_generating' ? fallbackStage : currentStage,
                blueprint_task_id: taskId,
  ...
  async generateEpisodeScript(input: GenerateEpisodeScriptInput) {
    ...
    const [episode] = await this.databaseService.db
      ...
      .limit(1)
    if (!episode) throw new NotFoundException('episode_not_found')
    if (!episode.blueprintPayload) throw new BadRequestException('episode_blueprint_required')
    ...
    const [drama] = await this.databaseService.db
      .select()
      .from(dramas)
      .where(and(eq(dramas.id, episode.dramaId), eq(dramas.userId, input.userId), isNull(dramas.deletedAt)))
      .limit(1)
    if (!drama) throw new NotFoundException('drama_not_found')
    ...
    const effectiveSourceTrace = sourceTrace.length ? sourceTrace : blueprint?.source_trace || []
    ...
    try {
      ...
      if (useRemoteAgent) {
        ...
        const run = await this.recordRemoteRun({
          ...
          mode: input.rewrite ? 'episode_script_rewrite' : 'episode_script_generate',
        ...
        scriptContent = this.buildLocalPilotScript(blueprint, episode.episodeNumber)
        latestRunId = run.id
      }
  ...
  private async generatePilotScriptsNow(input: GeneratePilotScriptsInput & { taskId?: number; episodeIds?: number[] }) {
    const { drama, metadata, aiFirst, targets } = await this.loadPilotScriptsContext(input)
    ...
    await this.databaseService.db
      ...
      .where(and(eq(episodes.userId, input.userId), inArray(episodes.id, targets.map((episode) => episode.id))))
    ...
    const updateProgress = async (phase: string, currentEpisodeNumber?: number | null) => {
      if (!input.taskId) return
      const progress = Math.min(96, 8 + Math.round(((successCount + failedCount) / targets.length) * 84))
      await this.updatePilotScriptsTask(input.taskId, {
        status: 'running',
        progress,
        resultSummaryJson: JSON.stringify({
    ...
    if (useRemoteAgent) {
      ...
      if (!successCount) throw new BadRequestException('pilot_script_generation_failed')
    } else if (this.shouldUseLocalRuleFallback()) {
      const run = await this.recordLocalRun({
        userId: input.userId,
        dramaId: input.dramaId,
      ...
      for (const episode of targets) {
        ...
        await updateProgress('episode_script', episode.episodeNumber)
        const blueprint = this.parseBlueprint(episode.blueprintPayload)
        const script = this.buildLocalPilotScript(blueprint, episode.episodeNumber)
        await this.databaseService.db
          .update(episodes)
          .set({
            content: script,
      ...
      .set({
        metadata: JSON.stringify({
          ...
          ai_first: {
            ...
            ai_first_stage: 'in_production',
            latest_script_ai_run_id: latestRunId,
            pilot_scripts_task_id: input.taskId || toNumberValue(aiFirst.pilot_scripts_task_id) || null,
            pilot_scripts_task_status: input.taskId ? 'completed' : toStringValue(aiFirst.pilot_scripts_task_status),
  ...
  private async loadPilotScriptsContext(input: GeneratePilotScriptsInput & { episodeIds?: number[] }): Promise<PilotScriptsContext> {
    const [drama] = await this.databaseService.db
      .select()
      .from(dramas)
      .where(and(eq(dramas.id, input.dramaId), eq(dramas.userId, input.userId), isNull(dramas.deletedAt)))
    ...
    const episodeRows = await this.databaseService.db
      ...
      .where(and(eq(episodes.dramaId, input.dramaId), eq(episodes.userId, input.userId), isNull(episodes.deletedAt)))
      .orderBy(episodes.episodeNumber)
    const targetIds = input.episodeIds?.length ? new Set(input.episodeIds) : null
    const targets = episodeRows
      .filter((episode) => episode.blueprintPayload && !episode.scriptContent?.trim())
      .filter((episode) => !targetIds || targetIds.has(episode.id))
      .slice(0, limit)

    if (!targets.length) throw new BadRequestException('episode_blueprint_required')
    return { drama, metadata, aiFirst, targets, limit }
  }

  private async createOrReusePilotScriptsTask(context: PilotScriptsContext) {
    const timestamp = new Date()
    const targetEpisodeNumbers = context.targets.map((episode) => episode.episodeNumber)
    const targetEpisodeIds = context.targets.map((episode) => episode.id)
    const title = compactText(`AI-first 试播正文：${context.drama.title}`, 120)
    const payload = {
  ...
  private async markPilotScriptsTaskQueued(context: PilotScriptsContext, taskId: number) {
    const timestamp = new Date()
    ...
    await this.databaseService.db
      ...
      .where(and(eq(episodes.userId, userId), inArray(episodes.id, episodeIds)))
  ...
  private async updatePilotScriptsTask(taskId: number, values: Partial<typeof tasks.$inferInsert>) {
    await this.databaseService.db
      .update(tasks)
      .set({
        ...values,
  ...
  private async completePilotScriptsTask(taskId: number, summary: Record<string, unknown>) {
    const timestamp = new Date()
    const [task] = await this.databaseService.db
      .select()
      .from(tasks)
  ...
  async failPilotScriptsTask(taskId: number, error: unknown) {
    ...
    if (episodeIds.length) {
      await this.databaseService.db
        ...
        .where(and(eq(episodes.userId, task?.userId ?? 0), inArray(episodes.id, episodeIds), eq(episodes.status, 'script_generating')))
    }
  ...
  private async recordLocalRun(input: {
    ...
    const [run] = await this.databaseService.db
      ...
      .values({
        ...
        assistantMessage: 'Internal demo fallback only: local rule executor produced a structured seed for integration testing. Not acceptable as formal 0.23.1 AI output.',
        referencesJson: JSON.stringify([{ kind: 'internal_demo', title: 'DRAMA_AI_FIRST_LOCAL_RULE_FALLBACK', reason: 'Local rule fallback is gated and must not be used for formal release acceptance' }]),
  ...
  private async shouldUseRemoteAgent(userId: number) {
    return Boolean(this.dramaAgentService && await this.dramaAgentService.canExecute(userId))
  }
  ...
  private async ensureSourceChunks(
    ...
    const existing = await this.databaseService.db
      ...
      .where(and(eq(dramaSourceChunks.sourceId, source.id), eq(dramaSourceChunks.userId, source.userId)))
  ...
  private async analyzeLongSourceWithRemoteAgent(input: {
    ...
    const chunkAnalyses: Array<SourceChunkAnalysisPayload & { chunk_id: number; chunk_no: number }> = []
    const reportChunkProgress = async (phase: 'chunk_analysis' | 'global_summary' = 'chunk_analysis') => {
      if (!input.taskId) return
      const readyChunks = chunkAnalyses.length
      const progress = phase === 'global_summary'
        ? 88
    ...
    for (const chunk of chunks) {
      ...
      if (chunk.status === 'ready' && existingPayload.summary) {
        chunkAnalyses.push({
          ...
          key_events: Array.isArray(existingPayload.key_events) ? existingPayload.key_events.map(String) : [],
          characters: Array.isArray(existingPayload.characters) ? existingPayload.characters.map(String) : [],
          scenes: Array.isArray(existingPayload.scenes) ? existingPayload.scenes.map(String) : [],
          risks: Array.isArray(existingPayload.risks) ? existingPayload.risks.map(String) : [],
          source_trace: Array.isArray(existingPayload.source_trace) ? existingPayload.source_trace.map((item) => toRecord(item)) : parseJsonArray(chunk.sourceTrace),
      ...
      await this.databaseService.db
        ...
        .where(and(eq(dramaSourceChunks.id, chunk.id), eq(dramaSourceChunks.userId, input.userId)))

      const sourceTrace = parseJsonArray(chunk.sourceTrace)
      const chunkContent = input.source.content.slice(chunk.contentStart, chunk.contentEnd)
      try {
        ...
        const payload: SourceChunkAnalysisPayload & { chunk_id: number; chunk_no: number } = {
          ...agent.chunkAnalysis,
          source_trace: agent.chunkAnalysis.source_trace?.length ? agent.chunkAnalysis.source_trace : sourceTrace,
          ai_run_id: run.id,
          remote_run_id: agent.remoteRunId,
  ...
  private async recordRemoteRun(input: {
    ...
    const [run] = await this.databaseService.db
      ...
      .values({
        ...
        referencesJson: JSON.stringify([{ kind: 'remote_run', title: input.remoteRunId, reason: 'RemoteDramaAgentAdapter execution' }]),
        actionsJson: JSON.stringify([]),
        createdAt: new Date(),
        updatedAt: new Date(),
      })
  ...
  private buildLocalSourceAnalysis(input: {
    ...
    const antagonist = characterNames.find((name) => name !== protagonist) || null
    ...
    return {
      ...
      relationship_map: characterNames.slice(0, 4).map((name, index) => ({
        character: name,
        role: index === 0 ? 'protagonist_candidate' : 'supporting_or_opposing_force',
        evidence: createSourceTrace(input.sourceId, input.health, index),
      })),
      world_rules: [
        input.health.chapter_count > 1 ? `源稿共识别 ${input.health.chapter_count} 个章节，可按章节弧线改编。` : '源稿缺少明确章节结构，需要按全文节拍拆分。',
        input.health.over_context_limit ? '源稿超过安全上下文阈值，必须通过分块摘要合成全局理解。' : '源稿可直接进入策略生成。',
      ],
      emotional_curve: chapterIndex.slice(0, 6).map((chapter, index) => ({
        chapter_no: chapter.chapter_no,
        title: chapter.title,
        position: index === 0 ? 'setup' : index === chapterIndex.length - 1 ? 'resolution' : 'escalation',
        brief: chapter.brief,
      })),
      adaptation_risks: [
        input.health.status === 'warning' ? '源稿健康检查存在 warning，策略生成前建议人工复核。' : '',
        input.health.over_context_limit ? '长篇源稿需要保留 source trace，避免只改编前段内容。' : '',
      ].filter(Boolean),
      evidence: [
        {
  ...
  private buildLocalAdaptationBriefs(input: {
    ...
    const episodeCount = Math.min(80, Math.max(3, input.targetEpisodeCount || Math.ceil(Math.max(input.health.chapter_count, 3) * 1.5)))
    ...
    return variants.slice(0, input.count).map((variant, index) => ({
      ...
      retained_points: [
        ...
        ...(input.health.chapter_index || []).slice(0, 2).map((chapter) => chapter.title),
      ].filter(Boolean),
      removed_points: ['弱支线', '重复解释段落', '不服务主线的长铺垫'],
      risk_notes: [
        ...input.analysis.adaptation_risks,
        '本地规则执行器仅生成结构化种子，封版前应替换为远程 Agent 或真实 AI 输出。',
      ],
  ...
  private buildLocalEpisodeBlueprints(input: {
    ...
    const chapterIndex = input.health.chapter_index || []
    const total = Math.min(120, Math.max(1, input.brief.target_episode_count || chapterIndex.length || 1))
    const characters = [input.analysis.protagonist, input.analysis.antagonist || '关键对手'].filter(Boolean) as string[]
    ...
    return Array.from({ length: total }).map((_, index) => {
      ...
      const titleSuffix = episodeNumber === 1
        ? '开局钩子'
        : episodeNumber === total
          ? '终局反转'
          : '冲突升级'
      return {
        ...
        positioning: episodeNumber <= 3 ? '试播关键集' : episodeNumber === total ? '收束集' : '主线推进集',
        opening_hook: episodeNumber === 1
          ? compactText(input.analysis.core_conflict, 80)
        ...
        ending_hook: episodeNumber === total ? '主线阶段性闭合，保留续作余味。' : '在答案揭晓前切断，形成下一集点击动力。',
  ...
  private parseBlueprint(value: string | null | undefined): EpisodeBlueprintPayload | null {
    if (!value) return null
    try {
  ...
  private buildLocalPilotScript(blueprint: EpisodeBlueprintPayload | null, episodeNumber: number) {
    ...
    const characters = blueprint?.characters?.length ? blueprint.characters.join('、') : '核心角色'
    ...
    return [
      ...
      `【结尾悬念】${endingHook}`,
      '',
      '> 本正文由本地规则执行器生成，用于打通试播正文与工作台输入；封版前应替换为真实 AI/远程 Agent 生成结果。',
    ].join('\n')
  }

  private buildAiFirstTaskSummary(task: typeof tasks.$inferSelect): AiFirstTaskSummary {
  ...
  private async buildAiFirstPayload(drama: typeof dramas.$inferSelect) {
    ...
    const [source] = Number.isInteger(sourceId) && sourceId > 0
      ? await this.databaseService.db
        .select()
        .from(dramaSources)
        .where(and(eq(dramaSources.id, sourceId), eq(dramaSources.dramaId, drama.id), isNull(dramaSources.deletedAt)))
        .limit(1)
      : await this.databaseService.db
        .select()
        .from(dramaSources)
        .where(and(eq(dramaSources.dramaId, drama.id), isNull(dramaSources.deletedAt)))
        .orderBy(desc(dramaSources.createdAt))
        .limit(1)
    ...
    return {
      ...
      adaptation_briefs: Array.isArray(aiFirst.adaptation_briefs) ? aiFirst.adaptation_briefs : [],
      ...
      source_analysis_task: sourceAnalysisTask
        ...
        : null,
      brief_task: briefTask
        ? this.buildAiFirstTaskSummary(briefTask)
        : null,
