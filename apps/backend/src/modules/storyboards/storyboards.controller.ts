...
function toStoryboardDraftPreviewResponse(draft: StoryboardDraftPreview) {
  return {
    set_id: draft.id,
    drama_id: draft.dramaId,
    episode_id: draft.episodeId,
    revision: draft.revision,
    status: draft.status,
    origin: draft.origin,
    source_task_id: draft.sourceTaskId,
    source_execution_id: draft.sourceExecutionId,
    base_revision: draft.baseRevision,
    base_content_hash: draft.baseContentHash,
    created_at: draft.createdAt,
    updated_at: draft.updatedAt,
    current_baseline: {
      active_set_id: draft.currentBaseline.activeSetId,
      revision: draft.currentBaseline.revision,
      content_hash: draft.currentBaseline.contentHash,
      storyboard_count: draft.currentBaseline.storyboardCount,
      has_legacy_rows: draft.currentBaseline.hasLegacyRows,
      has_mixed_sets: draft.currentBaseline.hasMixedSets,
      human_edited_at: draft.currentBaseline.humanEditedAt,
      has_produced_media: draft.currentBaseline.hasProducedMedia,
    },
    items: draft.items.map((item) => ({
      ...
      bgm_prompt: item.bgm_prompt ?? null,
      sound_effect: item.sound_effect ?? null,
      duration: item.duration,
      scene_id: item.scene_id,
      character_ids: item.character_ids,
      opening_state: item.opening_state ?? null,
      closing_state: item.closing_state ?? null,
      continuity_to_next: item.continuity_to_next ?? null,
    })),
  };
}
...
export class StoryboardsController {
  ...
  private async syncStoryboardCharacters(
    storyboardId: number,
    characterIds: number[],
  ) {
    await this.databaseService.db
  ...
  private async validateStoryboardBindings(
    episodeId: number,
    sceneId: number | null,
    characterIds: number[],
  ) {
    ...
    const invalidCharacterIds = characterIds.filter(
      (characterId) => !episodeCharacterIds.has(characterId),
    );
  ...
  async listEpisodeStoryboards(
    ...
    const episodeCharacterIdsSet = new Set(
  ...
  @Get("episodes/:id/storyboard-draft")
  async getLatestStoryboardDraft(
    @Param("id") id: string,
    @CurrentUser() currentUser: CurrentUserType,
  ) {
    const episodeId = parseId(id, "episode id");
    const draft = await this.storyboardSetsService.getLatestReviewRequiredDraft(
      {
    ...
    return {
      draft: draft ? toStoryboardDraftPreviewResponse(draft) : null,
    };
  }
  ...
  @Post("storyboard-sets/:id/publish")
  async publishStoryboardSet(
    @Param("id") id: string,
    @Body() body: Record<string, unknown>,
    @CurrentUser() currentUser: CurrentUserType,
  ) {
    const storyboardSetId = parseId(id, "storyboard set id");
    const result = await this.storyboardSetsService.publishCurrentAgentDraft({
      userId: currentUser.id,
      storyboardSetId,
      confirmReplace: Boolean(body.confirm_replace),
    });
    return {
      set_id: result.setId,
      revision: result.revision,
      status: result.status,
      storyboard_count: result.storyboardCount,
      requires_review: result.requiresReview,
    };
  }

  @Get("storyboard-sets/:id")
  async getStoryboardSet(
    @Param("id") id: string,
    @CurrentUser() currentUser: CurrentUserType,
  ) {
    const storyboardSetId = parseId(id, "storyboard set id");
    const draft = await this.storyboardSetsService.getStoryboardSetPreview({
      userId: currentUser.id,
      storyboardSetId,
    });
    return toStoryboardDraftPreviewResponse(draft);
  }
  ...
  async updateStoryboard(
    ...
    const storyboard = await this.requireOwnedStoryboard(
      storyboardId,
      currentUser.id,
    );
    ...
    const characterIds = Object.prototype.hasOwnProperty.call(
      body,
      "character_ids",
    )
      ? toNumberArray(body.character_ids)
      : (await this.getStoryboardCharacterIdsMap([storyboardId])).get(
          storyboardId,
        ) || [];
    ...
    await this.validateStoryboardBindings(
    ...
    if (
      !hasUpdates &&
      !Object.prototype.hasOwnProperty.call(body, "character_ids")
    ) {
    ...
    await this.storyboardSetsService.markHumanEdited(
      storyboardId,
      currentUser.id,
    );
