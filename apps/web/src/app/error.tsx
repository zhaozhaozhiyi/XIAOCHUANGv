...
import { Clapperboard, RotateCcw, Mail } from 'lucide-react'
...
export default function ErrorPage({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-bg-page px-6 text-center">
      ...
      {/* Message */}
      <h1 className="mt-4 font-display text-2xl font-semibold text-text-0">服务出错了</h1>
      <p className="mt-2 max-w-[320px] text-sm text-text-3 leading-relaxed">
        服务器遇到了意外问题，我们正在修复中。你可以尝试刷新页面。
      </p>
      {error.digest && (
        <p className="mt-2 font-mono text-xs text-text-3/60">
          错误码：{error.digest}
        </p>
      )}
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
        <button
          onClick={reset}
          className="inline-flex items-center gap-2 rounded-full bg-accent px-5 py-2.5 text-sm font-medium text-on-accent shadow-primary-glow transition-all hover:bg-accent-dark cursor-pointer"
        >
          <RotateCcw size={14} />
          重试
        </button>
