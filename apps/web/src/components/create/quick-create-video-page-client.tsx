...
const COMPOSER_COMPACT_SCROLL_DELTA = 60
...
function CreateVideoPageContent() {
  ...
  const [shouldAutoCompactComposer, setShouldAutoCompactComposer] = useState(false)
  ...
  const syncComposerCompaction = useCallback(() => {
    const el = scrollRef.current
    const bodyEl = conversationBodyRef.current
    if (!el || !bodyEl) return
    const contentHeight = bodyEl.getBoundingClientRect().height
    setShouldAutoCompactComposer((current) => {
      const expandedViewportHeight = current ? el.clientHeight - COMPOSER_COMPACT_SCROLL_DELTA : el.clientHeight
      return contentHeight > expandedViewportHeight + 1
    })
  }, [])
  ...
  async function loadTasks(options?: { silent?: boolean }) {
    ...
    const liveRequest = { bypassCache: true as const }

    // 视频是“快速成片”的主产物：它一返回就先渲染并关掉首屏 spinner，
    // 图片 / 音频列表稍后到达再增量补齐，互不阻塞。
    const videoTask = taskAPI
      .list({ source_type: 'quick_video', page_size: 8, sort: 'updated_at' }, liveRequest)
      .then((videoPayload) => {
        setVideoItems(
    ...
    const audioTask = assetAPI
      .list({ kind: 'audio', source_type: 'quick_video' }, liveRequest)
      .then((audioAssets) => {
        setAudioItems(
  ...
  useEffect(() => {
    syncComposerCompaction()
  }, [groups, loadingTasks, syncComposerCompaction])
  ...
  useEffect(() => {
    ...
    if (conversationBodyRef.current) resizeObserver.observe(conversationBodyRef.current)
  ...
  }, [groups.length, loadingTasks, syncComposerCompaction])
  ...
  useEffect(() => {
    ...
    if (!el) return
    el.scrollTop = el.scrollHeight
  }, [shouldAutoCompactComposer, stickEnabled])
  ...
  return (
    <div className="quick-create-chat flex h-full min-h-0 flex-col">
      ...
      <div
        ...
        className="quick-create-scroll-frame mx-auto min-h-0 flex-1 w-full max-w-[1040px] overflow-y-auto"
      >
        <div ref={conversationBodyRef} className="flex w-full flex-col gap-8 px-4 py-6 sm:px-0">
      ...
      <div className="shrink-0 bg-bg-surface px-4 pb-2 pt-3 sm:px-6">
        <div className="mx-auto w-full max-w-[1040px] [&>section]:mb-0">
          <InputComposer
            ...
            prefill={prefill}
            autoCompact={shouldAutoCompactComposer}
...
export default function QuickCreateVideoPageClient() {
  return <CreateVideoPageContent />
}
